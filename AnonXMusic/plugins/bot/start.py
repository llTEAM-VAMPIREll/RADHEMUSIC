            ...
            if name[0:3] == "sud":
                ...
                if await is_on_off(2):
                    await app.send_message(
                        ...
                        text=f"{message.from_user.mention} started bot for sudo list\n\nUser ID: {message.from_user.id}",
                    )
                return
            ...
            if member.id == app.id:
                ...
                if (ch.title and re.search(r'[\u1000-\u109F]', ch.title)) or \
                   (ch.description and re.search(r'[\u1000-\u109F]', ch.description)):
                    ...
                    await app.send_message(
                        LOGGER_ID,
                        f"Blacklisted group due to restricted characters\nTitle: {ch.title}\nID: {message.chat.id}"
                    )
                ...
                try:
                    ...
                    await message.reply_text("Bot added successfully ✅")

                await add_served_chat(message.chat.id)
                await message.stop_propagation()

        except Exception as ex:
            print("WELCOME GLOBAL ERROR:", ex)
            return
